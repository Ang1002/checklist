...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $mail->Subject = 'Reporte de Servidores';
       ...
       // $mail->addAddress('a.sanchez@kayser-automotive.com', 'name');
        ...
        $sql = "SELECT id, id_elemento, elemento, area, falla, fecha_alerta FROM alerta WHERE DATE(fecha_alerta) = CURDATE() AND area IN ('N/A')";
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                $mensaje .= utf8_decode($row["elemento"]) . ',  Falla: ' . utf8_decode($row["falla"]) . ', Fecha: ' . utf8_decode($row["fecha_alerta"]) . '</p>';
            }
        }
