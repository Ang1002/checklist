...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Obtener el nombre y la URL de las fotos adjuntas
    $foto_nombre = $_FILES['foto']['name'];
    $foto_urls = $_FILES['foto']['tmp_name'];

    // Insertar datos en la tabla de la base de datos
    $sql = "INSERT INTO reportes (comentarios) VALUES ('$comentarios')";
    if ($conn->query($sql) === TRUE) {
        $reporte_id = $conn->insert_id; // Obtener el ID del reporte recién insertado

        // Insertar nombres y URLs de fotos adjuntas en la tabla de la base de datos
        foreach ($foto_nombre as $key => $nombre) {
            $url = $foto_urls[$key];
            $sql_foto = "INSERT INTO reporte_fotos (reporte_id, foto_nombre, foto_url) VALUES ($reporte_id, '$nombre', '$url')";
    ...
    // Ahora procedemos a enviar el correo electrónico
    ...
    try {
      ...
      // $mail->addAddress('N.Kayser@kayser-automotive.com', 'name');
        ...
        $mensaje = '<p>Comentarios:  ' . $comentarios . '</p>';
        if ($result->num_rows > 0) {
            $mensaje .= '<p>Alertas de hoy:</p>';
            while($row = $result->fetch_assoc()) {
                $mensaje .= ($row["elemento"]) . ',  Area: ' . ($row["area"]) . ',  Falla: ' .($row["falla"]) . ', Fecha: ' . ($row["fecha_alerta"]) . '</p>';
            }
        }
...
<body>
    ...
    <form name="form1" method="post" enctype="multipart/form-data">
        ...
        <p align="center">
            ...
            <input class="buttom" type="button" value="Volver" onclick="window.location.href='check.php'">
        </p>
    </form>
